...
(function () {
  ...
  function setupCoverEntrance() {
    ...
    if (prefersReducedMotion) {
      [
        '.hse-cover__kicker', '.hse-cover__headline', '.hse-cover__body',
        '.hse-cover__ctas', '.hse-cover__colophon', '.hse-cover__scroll-cue'
  ...
  function setupFullbleedQuoteA11y() {
    document.querySelectorAll('.hse-section--fullbleed-quote').forEach(function (s) {
  ...
  function setupStickyStepsA11y() {
    ...
    var firstImgs = document.querySelectorAll('.hse-ss__visual-img:first-child');
    firstImgs.forEach(function (img) { img.classList.add('is-active'); });
  }
  ...
  function setupRevealCrossfade() {
    gsap.utils.toArray('.hse-section--reveal-crossfade').forEach(function (section) {
      ...
      phases.forEach(function (phase, i) {
        ScrollTrigger.create({
          trigger: phase, start: 'top 65%', once: true,
          onEnter: function () {
  ...
  function setupRevealCrossfadeA11y() {
    ...
    document.querySelectorAll('.hse-section--reveal-crossfade').forEach(function (s) {
  ...
  function setupParallax() {
    gsap.utils.toArray('.hse-parallax__img').forEach(function (img) {
      ...
      gsap.to(img, {
        yPercent: 20 * speed * 10, ease: 'none',
        scrollTrigger: {
    ...
    document.querySelectorAll('.hse-toggle').forEach(function (toggle) {
      ...
      buttons.forEach(function (btn, i) {
        ...
        btn.addEventListener('keydown', function (e) {
          ...
          if (next >= 0) { e.preventDefault(); activateTab(next); buttons[next].focus(); }
        });
      });
  ...
  function setupOdometer() {
    document.querySelectorAll('.hse-stat[data-odometer="true"]').forEach(function (stat) {
      ...
      var number   = raw.replace(prefix, '').replace(suffix, '').replace(/,/g, '');
      ...
      function formatNumber(val) {
        var rounded = isFloat ? val.toFixed(decimals) : Math.round(val).toString();
        if (hasCommas) {
